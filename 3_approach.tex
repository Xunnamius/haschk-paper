...
through a quarter of \emph{malware and phishing warnings} and \emph{70\% of TLS
...
similarly \emph{determined} to transact resources over the internet, will not be
...
individual hosted resources and 2) a highly available mapping of unique
...
Note that we consider URNs as an alternative fingerprinting scheme combining RIs
and NACs in the discussion (cf. \secref{discussion}).
...
appear as the third-level domain following the OD in any request to the backend.
The RI Sub-Label (SL) is a standard string used to identify DNS records that
contain RIs; we used ``\_ri'' in our implementation. The resulting construction,
