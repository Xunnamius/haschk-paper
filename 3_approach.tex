...
The plugin computes RIs from the full URL path of a resource. For example,
...
in our implementation. For example: \emph{somesite.com} would be the OD for the
URL \emph{frag.something.somesite.com} and \emph{fakesite.io} would be the OD
for the URL \emph{git.fakesite.io}.
...
appear as the third-level domain following the OD. The RI Sub-Label (SL) is a
standard string used to identify DNS records that contain RIs; we used ``\_ri''
in our implementation. This string, consisting of \texttt{SL.PL.OD}, is 
