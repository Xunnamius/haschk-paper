...
In the case where NAC Validation fails, \ie they do not match, the user will be
alerted. Otherwise, \SYSTEM{} remains completely transparent the the end user,
as demonstrated by our browser-based implementations.
...
when considering the primary consumers of those resources---end users---who
likely cannot distinguish a digital signature from, for instance, a
key~\cite{PGPBad}. Further, unlike a standardized approach like HTTPS/TLS or
DNSSEC, which are notoriously hard to implement correctly in their own right,
rolling your own PKI-based solution is a path fraught with even greater peril.
It is well known in the community that these often very complex PKI systems are
hard to design correctly, hard implement correctly, and hard to effectively
maintain.
