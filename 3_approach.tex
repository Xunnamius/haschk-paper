...
requires no source code or user-facing web server/infrastructure changes, does
not employ unreliable heuristics, does not interfere with other software that
also handles download security, and can be immediately and transparently
deployed without breaking clients that do not implement the \SYSTEM{} approach.
Further, though our concrete implementation relies on DNS and DNS Security
(DNSSEC), the approach itself is agnostic of its ``backend,'' be it DNS,
Distributed Hash Tables (DHT), distributed key-value stores such as Redis, et
cetera.
