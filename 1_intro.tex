...
\SYSTEM{}'s utility}. We divide this risk into three categories: response
authentication, communication confidentiality, and resource integrity.
...
is outside of the model addressed by TLS and HTTPS~\cite{TLS1.2, HTTPS}; \ie
Protected encrypted communications mean nothing if the contents of those
communications are themselves corrupted before the fact.

More broadly, attacks on resource integrity are known as \textbf{Supply Chain
...
goal of infecting and exploiting one or more victims.
...
This can occur despite the integrity guarantee provided by TLS/HTTPS because, 
...
Unfortunately, checksums are not a viable solution. \TODO{(expound on ineffectiveness;
