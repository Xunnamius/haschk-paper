...
consequences~\cite{PGPBad} (see: \secref{background}). \\
...
Further, DNSSEC adoption across is small and slow. Worldwide, less than 14
percent of DNS requests have DNSSEC validated by the resolver~\cite{APNIC} but
thanks to community initiatives is on the rise~\cite{Cloudflare} (use graph as
...
verification of large files, i.e. if the file is big, we don't have to wait for
it to finish downloading before we render a failing judgement. This saves the
user time. Perhaps using the Tiger hash, since Tiger Merkle Trees seem to be
popular among large P2P and file sharing applications.

\subsubsection{Uniform Resource Names in lieu of Resource Identifiers}

The goal of the Resource Identifiers (RI) is for them to essentially be Uniform
Resource Names (URN). Perhaps we can adoption a URN scheme that already exists,
like magnet links? We can hash those, and ensure our naming scheme is based on
file contents rather than its location on a web server, as is the case with
Resource Identifiers.

(adapt the rest of the wiki entry)
