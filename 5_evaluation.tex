...
call---during HotCRP's paper submission process. We find a DNS record addition
or update during the resource deployment process to be simple enough for service
administrators to implement and presents no significant burden to adoption
outside of DNS API integration into an organization's software development
supply chain.
...
performance networks~\cite{EDNS}, but this is an artifact of DNSSEC rather than
\SYSTEM{}~\cite{DNSSEC}. Further, there are no practical limits on the
scalability of a DNS zone itself, by which we mean zone file size is
unrestricted by the protocol. A service can host tens of thousands of resource
